...
export default async function build(): Promise<void> {
  ...
  const modelTypes = Array.from(
    new Set(
      modelFilePaths.map((filePath) => filePath.match(extractModelType)[1])
    ).values()
  );
  // TODO: split into loader function
  //Register partials in project for use in any template.
  const partials = await Promise.all(
    partialsPaths.map((filePath) => readFile(filePath))
  );
  partialsPaths.forEach((filePath, i) =>
  ...
  helpers.forEach((helper) => {
    Handlebars.registerHelper(helper.name, helper.helperFn);
  });

  await Promise.all(
    modelTypes.map(async (type) => {
      const jsonSchemaPath = jsonSchemaPaths.find((filePath) =>
        filePath.includes(`${type}.schema.json`)
      );
      const templatePath = templatePaths.find((filePath) =>
        filePath.includes(`${type}.hbs`)
      );
      const filePaths = modelFilePaths.filter((filePath) =>
        filePath.includes(`./models/${type}/`)
      );
      const scriptPath = preprocessingScriptPaths.find((filePath) =>
        filePath.includes(`${type}.js`)
      );
      ...
      if (filePaths.length && templatePath && jsonSchemaPath) {
        ...
        // Pre-processing step.
        if (scriptPath) {
          const preprocessingFns = await import(
        ...
        const preprocessedFilePaths = await Promise.all(
          filePaths.map((filePath) => preprocess(filePath, preprocessFn))
        );
        // Generate output step.
        const templateStr = await (await readFile(templatePath)).toString();
        const template = Handlebars.compile(templateStr);
        return Promise.all(
          preprocessedFilePaths.map((filePath) =>
            generate(filePath, template, postprocessFn)
          )
        );
      }
      return [];
